...
open class ClassifierModel () :
    LitePyTorchModel<IImageDetectionData, ImageDetectionArrayListOutput<String>>(),
    IClassifierModel<IImageDetectionData, ImageDetectionArrayListOutput<String>, String>
{
    constructor(modelPath: String? = null, classificationsJson: String? = null) : this() {
        if (modelPath != null) loadModel(modelPath)
        if (classificationsJson != null) loadClassifications(classificationsJson)
    }

    constructor(modelPath: String? = null, newClassifications: ClassificationSuperclassMap<String>? = null) : this() {
        if (modelPath != null) loadModel(modelPath)
        if (newClassifications != null) loadClassifications(newClassifications)
    }
